...
import { Terminal, Compass, Info, BookOpen, Layers, PlusCircle, LayoutDashboard, LogOut, Menu, X } from "lucide-react";
...
export default function Navbar() {
  ...
  // Simulated Auth State (Day 3 Better Auth flow-te update hobe)
  const [isLoggedIn, setIsLoggedIn] = useState(false); 

  // Requirement Match: Minimum 3 Public Routes for Logged Out users
  const publicRoutes = [
    { name: "Explore", href: "/projects/explore", icon: <Compass className="w-4 h-4" /> },
    { name: "About", href: "/about", icon: <Info className="w-4 h-4" /> },
    { name: "Blog", href: "/blog", icon: <BookOpen className="w-4 h-4" /> },
  ];

  // Requirement Match: Minimum 5 Routes for Logged In users[cite: 1]
  const privateRoutes = [
    { name: "Explore", href: "/projects/explore", icon: <Compass className="w-4 h-4" /> },
    { name: "Dashboard", href: "/dashboard", icon: <LayoutDashboard className="w-4 h-4" /> },
    { name: "Add Project", href: "/items/add", icon: <PlusCircle className="w-4 h-4" /> },
    { name: "Manage", href: "/items/manage", icon: <Layers className="w-4 h-4" /> },
        ...
        <div className="flex items-center gap-3">
          {isLoggedIn ? (
            <div className="flex items-center gap-3">
              ...
              <div className="w-8 h-8 rounded-full border border-brand-accent overflow-hidden hidden sm:block">
                <Image 
          ...
          <div className="px-4 pt-2 pb-6 space-y-2">
            {currentRoutes.map((route, index) => (
              <Link
                ...
                className="flex items-center gap-3 px-3 py-3 rounded-xl text-zinc-300 hover:text-brand-accent hover:bg-brand-muted/30 text-base font-medium transition-all"
              >
                {route.icon}
                {route.name}
              </Link>
